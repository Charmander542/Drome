...
// spotifySearchResult is a track hit returned by GET /spotify/search.
...
// searchTracks queries the Spotify Search API for tracks matching q.
func (c *spotifyClient) searchTracks(ctx context.Context, q string, limit int) ([]spotifySearchResult, error) {
	if limit <= 0 {
		limit = 20
	}
	if limit > 50 {
	...
	if err := c.apiGET(ctx, path, &resp); err != nil {
		return nil, err
	}
	out := make([]spotifySearchResult, 0, len(resp.Tracks.Items))
	for _, t := range resp.Tracks.Items {
		spotifyURL := t.ExternalURLs.Spotify
		if spotifyURL == "" {
			spotifyURL = "https://open.spotify.com/track/" + t.ID
		}
		out = append(out, spotifySearchResult{
			SpotifyID:  t.ID,
			Title:      t.Name,
			Artist:     joinArtists(t.Artists),
			Album:      t.Album.Name,
